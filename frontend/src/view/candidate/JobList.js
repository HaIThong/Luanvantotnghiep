    ...
    <div className="bg-cl pb-5" style={{ width:"100%"}}>
      ...
      >
        ...
        <div className="mt-4 d-flex flex-column ">
          ...
          <div className="flex-fill ps-3 mt-1 mt-lg-0" >
            <button
              type="submit"
              className="btn-job border-0  text-white rounded"
            >
              {isSearchLoading ? (
                <Spinner size="sm" />
              ) : (
      ...
      <div className="row row-cols-lg-3 mt-4 theem " style={{padding:"20px",borderRadius:"10px"}}>
        {jobs.length > 0 ? (
          jobs.map((job) => (
            <div
              className="mb-3 pointer1 "
              key={`job_${job.id}`}
              onClick={() => nav(`/jobs/${job.id}`)}
            >
              <div className="d-flex p-2 border hover-border-main hover-shadow-sm bg-white h-100 theem">
                <div
