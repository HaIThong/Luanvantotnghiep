    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div
          id="resume"
          ref={contentRef}
          className="mx-auto border mt-4 mb-5 p-0 shadow-sm border-1 border-dark"
          style={{ width: '800px' }}
        >
          <div className="position-relative mb-3 w-100">
            <div className="bg-minimal"></div>
            <div className="mt-0 d-flex flex align-items-center w-100 ">
              <div style={{ width: '75%' }}>
              <FlexInput 
                placeholder="HỌ TÊN"
                className="mt-1"
                innerClassName=" h1 cv-text-main1 text-uppercase "
                defaultValue={fullname}
                {...register('fullname')}
              />

              </div>
              <div style={{ width: '25%' }} className="text-centerim">
                <img
                  src={basicInfor?.avatar}
                  alt="avatar"
                  width="160px"
                  height="160px"
                  className="rounded-nopill" 
                  style={{marginTop:'9px'}}
                />
              </div>
            </div>
